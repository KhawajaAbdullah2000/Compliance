...
<div class="container">
    ...
    <h3 class="fw-bold text-center mt-2">Risk and Compliance Details by Asset Component for Service : {{$service_name}}
     Group: {{$group_name}} and Asset: {{$name}}
        ...
        <tbody>
            @foreach ($complianceData as $data)
                ...
                <tr>
                    ...
                    <td>
                        <a href="/risk_compliance_heatmap_for_asset_component_by_asset/{{ $project->project_id }}/{{$service_name}}/{{$group_name}}/{{$name}}/{{$data['component_name']}}/{{ auth()->user()->id }}" 
