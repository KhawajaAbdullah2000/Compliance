    ...
    <div class="border p-3" style="border: 1px solid #ccc; border-radius: 5px;">
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                @if ($org_projects->count() > 0 && in_array('Data Inputter', $permissions))
                    <div class="col-md-6">
                        <form action="/copy_assets/{{ $project_id }}/{{ auth()->user()->id }}" method="get"
        ...
        @if (in_array('Data Inputter', $permissions) && $page_type=="services_register")
            <a class="btn btn-success btn-md float-end mb-2"
                href="/iso_sec_2_1_new/{{ $project_id }}/{{ auth()->user()->id }}" role="button">Enter Service or
                Asset
...
{{-- 
            ...
            <tbody>
                @foreach ($data as $d)
                    <tr class="service-row" data-service="{{ $d->s_name }}" data-group="{{ $d->g_name }}"
                        data-name="{{ $d->name }}" data-c_name="{{ $d->c_name }}">
                        <td>{{ $d->s_name }}</td>
                        <td>{{ $d->g_name }}</td>
                        <td>{{ $d->name }}</td>
                        <td>{{ $d->c_name }}</td>
                           ...
                           @if($page_type=="risk_assessment")
                            ...
                            <td class="text-center">
                                <a href="/iso_sec_2_3_1_risk_selection/{{ $d->assessment_id }}/{{ $project_id }}/{{ auth()->user()->id }}"
                                    class="btn btn-primary btn-sm rounded-pill">Initiate</a>
  ...
  @if($page_type=="services_register")
        ...
        @if (
            (optional($complianceFramework)->framework_id == 2 || optional($complianceFramework)->framework_id == 6) &&
                optional($framework_approach)->framework_approach_types_id == 1 &&
                optional($risk_assessment_approach)->global_risk_assessment_approach_id == 1)
            <a href="/initiaite_risk_assessment_qual_event/{{ $project->project_id }}/{{ auth()->user()->id }}"
                class="btn btn-primary btn-md float-end">Manage Risk</a>
        @endif
