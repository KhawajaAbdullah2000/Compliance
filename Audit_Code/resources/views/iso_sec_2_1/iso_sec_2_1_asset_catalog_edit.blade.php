        ...
        <div class="col-lg-8">
            <div class="card shadow-lg border-0">
                ...
                <div class="card-body p-5">
                    <form action="/iso_sec_2_1_asset_catalog_submit_edit/{{ $data->id }}/{{ $data->organization_id }}/{{ auth()->user()->id }}" method="post">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            <label for="risk_confidentiality" class="form-label fw-semibold">Confidentiality Classification</label>
                            <select name="risk_confidentiality" class="form-select">
                                <option value="">-- Select --</option>
                                @foreach($riskValues as $val)
                                @php
                                $label = $isNamed ? ($riskMap[$val]['label'] ?? $val) : $val;
                                @endphp
                                <option value="{{ $val }}" {{ (old('risk_confidentiality', $data->risk_confidentiality) == $val) ? 'selected' : '' }}>
                                    {{ $isNamed ? "$val — $label" : $label }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            <label for="risk_integrity" class="form-label fw-semibold">Integrity Classification</label>
                            <select name="risk_integrity" class="form-select">
                                ...
                                $label = $isNamed ? ($riskMap[$val]['label'] ?? $val) : $val;
                                @endphp
                                <option value="{{ $val }}" {{ (old('risk_integrity', $data->risk_integrity) == $val) ? 'selected' : '' }}>
                                    {{ $isNamed ? "$val — $label" : $label }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            <label for="risk_availability" class="form-label fw-semibold">Availability Classification</label>
                            <select name="risk_availability" class="form-select">
                                ...
                                $label = $isNamed ? ($riskMap[$val]['label'] ?? $val) : $val;
                                @endphp
                                <option value="{{ $val }}" {{ (old('risk_availability', $data->risk_availability) == $val) ? 'selected' : '' }}>
                                    {{ $isNamed ? "$val — $label" : $label }}
                                </option>
                                @endforeach
                            </select>
