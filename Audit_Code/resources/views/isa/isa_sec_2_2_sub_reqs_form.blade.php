    ...
    <p>{{$filteredData[0][5]}} </p>
         ...
         <div class="container d-flex">
            <div class="card shadow-lg border-0 mt-2 mb-5" style="max-width: 1000px; width: 100%;">
                ...
                <div class="card-body">
                    <form action="{{ $isEditable ? "/isa_sec_2_2_form/$sub_req/$title/$project_id/".auth()->user()->id."/$asset->assessment_id" : '#' }}" method="post" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            @if(isset($result->attachment))
                                <p class="mt-3">Current Attachment: 
                                    <a href="{{ asset('isa_sec_2_2/'.$result->attachment) }}" download>{{ $result->attachment }}</a>
                                </p>
                            @endif
                        </div>
