...
@section('content')
    ...
    <div class="container">
        ...
        <div class="row mt-5">
            ...
            <div class="row mb-3">
                <div class="col-12 d-flex justify-content-between align-items-center">
                    <!-- Left: Title Box -->
                    <div class="p-3 text-white rounded shadow" style="background: linear-gradient(135deg, #FF512F, #DD2476);">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form
                        action="/update_risk_record/{{ $project->project_id }}/{{ auth()->user()->organization->id }}/{{ auth()->user()->id }}"
                        method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            <label class="fw-bold" for="catalog_select" class="form-label">Choose from Catalog
                                (optional)</label>

                            <div class="input-group">
                                <div class="input-group-text bg-white">
                                    <a href="/risk-description-catalog/{{$record->risk_id}}/{{ $project->project_id }}/{{auth()->user()->id}}" class="text-warning"
                                        title="Edit Catalog">
                            ...
                            <select name="risk_owner" id="risk_owner" class="form-select" required>
                                ...
                                @foreach ($risk_owners as $owner)
                                    <option value="{{ $owner->id }}"
                                        {{ $selectedValue === $owner->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            <label class="fw-bold" for="erm_risk_classification" class="form-label">ERM Risk
                                Classification</label>
                            <select name="erm_risk_classification" id="erm_risk_classification" class="form-select"
                                required>
                                ...
                                @foreach ($classifications as $classification)
                                    <option value="{{ $classification }}"
                                        {{ $selectedValue === $classification ? 'selected' : '' }}>
                                        {{ $classification }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            <label class="fw-bold" for="op_loss_event_type_one" class="form-label">Basel II Operational Loss
                                Event Type I</label>
                            <select name="op_loss_event_type_one" id="op_loss_event_type_one" class="form-select" required>
                                ...
                                @foreach ($eventTypes as $type)
                                    <option value="{{ $type }}"
                                        {{ $selectedEventType === $type ? 'selected' : '' }}>
                                        {{ $type }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            <label class="fw-bold" for="op_loss_event_type_two" class="form-label">Basel II Operational Loss
                                Event Type II</label>
                            <select name="op_loss_event_type_two" id="op_loss_event_type_two" class="form-select" required>
                                ...
                                @php
                                    ...
                                    $selectedEventTypeTwo = old(
                                        'op_loss_event_type_two',
                                        $record->op_loss_event_type_two,
                                    );
                                @endphp

                                @foreach ($eventTypesTwo as $type)
                                    <option value="{{ $type }}"
                                        {{ $selectedEventTypeTwo === $type ? 'selected' : '' }}>
                                        {{ $type }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="text-end">
            <a href="/initiate_gross_assessment_form/{{ $record->risk_id }}/{{ $project->project_id }}/{{ auth()->user()->id }}"
                class="btn btn-md mb-2 mt-4 fw-bold text-white"
                style="background: linear-gradient(135deg, #FF512F, #F09819); transition: 0.3s;">Initiate Gross Risk
                Assessment</a>
        </div>
