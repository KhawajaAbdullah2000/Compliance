        ...
        <form action="/save_likelihood_value_risk_treatment/{{$project->project_id}}/{{auth()->user()->id}}/{{$asset->assessment_id}}" method="POST">
            @csrf
             ...
             <label class="form-label fw-semibold mb-3">
                Likelihood Value before Risk Assessment : <span class="fs-4">{{$risk_assessment_likelihood_value}} </span>
            </label>
            <br>
            <label class="form-label fw-semibold mb-3">
                Select Likelihood Value for Risk Treatment 
            </label>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="form-check d-flex justify-content-center">
                                    <input required class="form-check-input" type="radio" name="likelihood_value" value="{{ $value }}" id="likelihood_{{ $value }}" {{ (isset($risk_treatment_likelihood_value) && $risk_treatment_likelihood_value == $value) ? 'checked' : '' }} {{ !$isEditable ? 'disabled' : '' }}>
                                </div>
                            </td>
                        </tr>
                        @endforeach
