...
<a href="/cobit_subsections/{{$project_id}}/{{auth()->user()->id}}/{{$asset->assessment_id}}" class="btn btn-primary btn-md float-end mb-4">Go to All Requirements</a>
        ...
        <p>{{$filteredData[0][4]}} </p>
         ...
         <div class="container d-flex">
            <div class="card shadow-lg border-0 mt-5 mb-5" style="max-width: 1000px; width: 100%;">
                ...
                <div class="card-body">
                    <form action="{{ $isEditable ? "/cobit_sec_2_2_form/$sub_req/$title/$project_id/".auth()->user()->id."/$asset->assessment_id" : '#' }}" method="post" enctype="multipart/form-data">
                        @csrf
