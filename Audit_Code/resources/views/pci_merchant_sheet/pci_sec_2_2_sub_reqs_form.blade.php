...
<a href="/pci_merchant_sec_2_2_subsections/{{$project_id}}/{{auth()->user()->id}}/{{$asset->assessment_id}}" class="btn btn-primary btn-md float-end mb-4">Go to All Requirements</a>
...
<h3>Select From below and apply to @if(Session('evidenceLevel')=='project') All Services and Assets in this Project @endif
    ...
    <div class="card shadow-lg border-0 mt-5">
        <div class="card-header bg-primary text-white text-center">
            <h3>View or Edit Compliance Status</h3>
        </div>
        <div class="card-body">
            <form action="/pci_merchant_sec_2_2_edit_form/{{$sub_req}}/{{$title}}/{{$project_id}}/{{auth()->user()->id}}/{{$asset->assessment_id}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
    ...
    <div class="card shadow-lg border-0 mt-5">
        <div class="card-header bg-success text-white text-center">
            <h3>Submit Compliance Details</h3>
        </div>
        <div class="card-body">
            <form action="/pci_merchant_sec_2_2_form/{{$sub_req}}/{{$title}}/{{$project_id}}/{{auth()->user()->id}}/{{$asset->assessment_id}}" method="post" enctype="multipart/form-data">
                @csrf
