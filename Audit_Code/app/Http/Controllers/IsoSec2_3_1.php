...
class IsoSec2_3_1 extends Controller
{
    ...
    public function iso_sec_2_3_1_risk_selection($asset_id, $proj_id, $user_id)
    {
        ...
        if ($user_id == auth()->user()->id) {
            ...
            if ($checkpermission) {
                ...
                $framework_approach_types = DB::table('framework_approach_types')->get();
                ...
                return view('iso_sec_2_3_1.show_selected_framework', [
                    ...
                    'risk_management_frameworks' => $risk_management_frameworks,
                    'framework_approach_types' => $framework_approach_types,
    ...
    public function iso_27005_submit_risk_assessment_qual_event($proj_id, $user_id, Request $req)
    {
        ...
        if (
            in_array($frameworkDetails['complianceFramework']->framework_selected, [2, 6])
            && ($frameworkDetails['framework_approach']->framework_approach_types_id == 1 || $frameworkDetails['framework_approach']->framework_approach_types_id == 2)
            && $frameworkDetails['risk_assessment_approach']->assessment_approach_selected == 1
        ) {
            return redirect()->route('iso_27005_risk_assessment_qual_event', [
    ...
    public function proj_asset_selected_level_of_threat_qual_event($proj_id, $user_id, Request $req)
    {
        ...
        if ($checkpermission) {
            ...
            if (
                ...
                if ($req->input('action') === 'save_and_stay') {
                    return redirect()->route("qual_event_consolidated_threat", [
                        'proj_id' => $checkpermission->project_id,
                        'user_id' => $user_id,
                    ])->with('success', 'Data Saved Successfully');
                }
    ...
    public function qual_event_consolidated_threat($proj_id, $user_id)
    {
        ...
        if ($checkpermission) {
            ...
            $global_level_of_threats = Db::table('global_level_of_threats')->orderby('global_level_of_threats_id', 'desc')
                ->get();

            $selected_level_of_threat = Db::table('proj_asset_selected_level_of_threat')->where('project_id', $proj_id)->first();


            return view('iso_27005.qual_event_consolidated_threat', [
                'project_id' => $checkpermission->project_id,
                'project_name' => $checkpermission->project_name,
                'project_permissions' => $checkpermission->project_permissions,
                'project' => $project,
                ...
                'services' => $services,
                'global_level_of_threats' => $global_level_of_threats,
                'selected_level_of_threat' => $selected_level_of_threat->threat_selected ?? null,
    ...
    public function update_framework_approach($proj_id, $user_id, Request $req)
    {
        ...
        $option = $validated['framework_option'];
        $org_id = auth()->user()->organization->id;
        ...
        switch ($option) {
            case 'qualitative_asset':
                $framework_selected = 2;
                $framework_approach_types_id = 1; // Qualitative
                $global_risk_assessment_approach_id = 2; // Asset-based
                break;

            case 'quantitative_asset':
                $framework_selected = 2;
                $framework_approach_types_id = 2; // Quantitative
                $global_risk_assessment_approach_id = 2; // Asset-based
                break;

            case 'qualitative_event':
                $framework_selected = 2;
                $framework_approach_types_id = 1;
                $global_risk_assessment_approach_id = 1; // Event-based
                break;

            case 'quantitative_event':
                $framework_selected = 2;
                $framework_approach_types_id = 2;
                $global_risk_assessment_approach_id = 1;
                break;

            case 'default':
            default:
                $framework_selected = 1; // Default framework
                $framework_approach_types_id = null;
                $global_risk_assessment_approach_id = null;
                break;
        }
        ...
        DB::table('org_projects_framework_selected')->updateOrInsert(
            ...
            [
                ...
                'updated_at' => now(),
                'created_at' => now(), // in case of insert
            ]
        );
        ...
        return redirect()->back()->with('success', 'Framework selection updated successfully.');
    }
}
